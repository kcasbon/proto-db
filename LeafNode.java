...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        for (int i =0; i < this.keys.size(); ++i) {
            if (key.equals(this.keys.get(i))) {
                String message = String.format("key %d already exists in this leaf.", key.getInt());
    ...
    // See BPlusNode.splitNode
    @Override
    public Optional<Pair<DataBox, Long>> splitNode() {
            LeafNode newLeaf = new LeafNode(metadata, bufferManager, new ArrayList<>(),
                    new ArrayList<>(), this.rightSibling, treeContext);
            int d = metadata.getOrder();
            for (int i = 0; i < d + 1; ++i) {
                newLeaf.keys.add(this.keys.get(d));
                newLeaf.rids.add(this.rids.get(d));
            ...
            // change this.leaf right sibling to newLeaf
            this.rightSibling = Optional.of(Optional.of(newLeaf.getPage().getPageNum()).orElse(-1L));
            Pair<DataBox, Long> splitKey = new Pair(newLeaf.keys.get(0), newLeaf.getPage().getPageNum());

            sync();
            newLeaf.sync();

            //System.out.println(newLeaf.toSexp());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            ...
            DataBox nextKey = nextPair.getFirst();
            RecordId nextRecord = nextPair.getSecond();
            ...
            if (keys.size() > Math.ceil(fillFactor * 2 * metadata.getOrder())) {
                return this.bulkLeafSplit();
            }
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLeafSplit() {
        LeafNode newLeaf = new LeafNode(metadata, bufferManager, new ArrayList<>(),
                new ArrayList<>(), Optional.empty(), treeContext);

        int keyLen = this.keys.size();

        newLeaf.keys.add(this.keys.get(keyLen - 1));
        newLeaf.rids.add(this.rids.get(keyLen - 1));

        this.keys.remove(keyLen - 1);
        this.rids.remove(keyLen - 1);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSib = Optional.of(Optional.of(buf.getLong()).orElse(-1L));
        if (rightSib.get().equals(-1L)) {
