...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        List<Integer> eligibleIndCols = getEligibleIndexColumns(table);
        int except = -1;

        for (int i = 0; i < eligibleIndCols.size(); i ++) {
            SelectPredicate currPred = selectPredicates.get(i);

            String tableName = currPred.tableName;
            String column = currPred.column;
            PredicateOperator operator = currPred.operator;
            DataBox value = currPred.value;

            IndexScanOperator currScan = new IndexScanOperator(transaction, tableName, column, operator, value);

            if (currScan.estimateIOCost() < minIOCost) {
                minIOCost = currScan.estimateIOCost();
                minOp = currScan;
                except = i;
            }
        }

        QueryOperator finalMinOp = addEligibleSelections(minOp, except);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Set<Set<String>> keySet = prevMap.keySet();
        ArrayList<Set<String>> keyList = new ArrayList<>(keySet);

        Collection<QueryOperator> opValues = prevMap.values();
        ArrayList<QueryOperator> newList = new ArrayList<>(opValues);

        for (int i = 0; i < prevMap.size(); i ++) {
            Set<String> getTables = keyList.get(i);
            HashSet<String> currTables = new HashSet<>(getTables);

            QueryOperator currOp = newList.get(i);
            ...
            for (int j = 0; j < this.joinPredicates.size(); j ++) {
                ...
                String leftTable = this.joinPredicates.get(j).leftTable;
                String rightTable = this.joinPredicates.get(j).rightTable;
                String leftCol = this.joinPredicates.get(j).leftColumn;
                String rightCol = this.joinPredicates.get(j).rightColumn;

                QueryOperator otherOp;
                Set<String> getSet = new HashSet<String>();
                QueryOperator nextMin;

                if (currTables.contains(leftTable) && !currTables.contains(rightTable)) {
                    getSet.add(rightTable);
                    otherOp = pass1Map.get(getSet);
                    gotLeft = false;

                } else if (!currTables.contains(leftTable) && currTables.contains(rightTable)) {
                    getSet.add(leftTable);
                    otherOp = pass1Map.get(getSet);
                } else {
                    continue;
                }
                ...
                if (gotLeft) {
                    nextMin = minCostJoinType(otherOp, currOp, leftCol, rightCol);
                } else {
                    nextMin = minCostJoinType(currOp, otherOp, leftCol, rightCol);
                }


                if (nextMin.estimateIOCost() < minCost) {
                    minOp = nextMin;
                    minCost = nextMin.estimateIOCost();
                    minIndex = j;
                    minLeft = gotLeft;
                }
            }
            ...
            if (minLeft) {
                currTables.add(this.joinPredicates.get(minIndex).leftTable);
            } else {
                currTables.add(this.joinPredicates.get(minIndex).rightTable);
            }


            result.put(currTables, minOp);
        }
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        for (int i = 0; i < this.tableNames.size(); i ++) {
            HashSet<String> nextTable = new HashSet<>();
            nextTable.add(this.tableNames.get(i));
            QueryOperator nextOp = minCostSingleAccess(this.tableNames.get(i));
            pass1Map.put(nextTable, nextOp);
        }
        ...
        ArrayList<QueryOperator> opList = new ArrayList<>(prevMap.values());
        finalOperator = opList.get(0);
