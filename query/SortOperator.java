...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        ArrayList<Record> sortedArr = new ArrayList<>();
        ...
        Collections.sort(sortedArr, comparator);

        Run finalRun = new Run(transaction, getSchema());

        for (int i = 0; i < sortedArr.size(); i ++) {
            finalRun.add(sortedArr.get(i));
        }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        ArrayList<BacktrackingIterator> iterators = new ArrayList<>();
        ...
        for (int i = 0; i < iterators.size(); i++) {
            if (iterators.get(i).hasNext()) {
                Pair<Record, Integer> addQueue = new Pair(iterators.get(i).next(), i);
                queue.add(addQueue);
            }
        }
        ...
        Run finalRun = new Run(transaction, schema);

        while (!queue.isEmpty()) {
            Pair<Record, Integer> nextVal = queue.poll();
            finalRun.add(nextVal.getFirst());

            Integer iterInd = nextVal.getSecond();

            if (iterators.get(iterInd).hasNext()) {
                Pair<Record, Integer> nextQueue = new Pair(iterators.get(iterInd).next(), iterInd);
                queue.add(nextQueue);
            }
        }
        return finalRun;
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int sizeOutputRuns = numBuffers - 1;
        float numInputRuns = runs.size();

        double numOutputRuns = Math.ceil(numInputRuns / sizeOutputRuns);
        List<Run> finalRuns = new ArrayList<>();

        Iterator<Run> runIter = runs.iterator();

        for (int i = 0; i < numOutputRuns; i++) {
            List<Run> nextRunList = new ArrayList<>();
            for (int j = 0; j < sizeOutputRuns; j ++) {
                if (runIter.hasNext()) {
                    nextRunList.add(runIter.next());
                }
            }
            finalRuns.add(mergeSortedRuns(nextRunList));
        }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            BacktrackingIterator recordIter = getBlockIterator(sourceIterator, schema, numBuffers);
            initRuns.add(sortRun(recordIter));
        }
