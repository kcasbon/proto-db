...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        BPlusTreeIterator iterAll = new BPlusTreeIterator(leftMost);
        iterAll.currRids = leftMost.scanAll();
        //System.out.println(iterAll.hasNext());
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        //consider using scanEqual
        LeafNode start = this.root.get(key);
        BPlusTreeIterator iterGreaterEqual = new BPlusTreeIterator(start, key);
        iterGreaterEqual.currRids = start.scanGreaterEqual(key);

        return iterGreaterEqual;
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> pushKey = this.root.put(key, rid);


        if (pushKey.isPresent()) {

            Long newLeftNode = this.root.getPage().getPageNum();
            //update root with new inner node
            List<DataBox> rootKeys = new ArrayList<>();
            List<Long> rootChildren = new ArrayList<>();

            rootKeys.add(pushKey.get().getFirst());
            rootChildren.add(newLeftNode);
            rootChildren.add(pushKey.get().getSecond());

            InnerNode newRoot = new InnerNode(metadata, bufferManager, rootKeys, rootChildren, lockContext);
            updateRoot(newRoot);

            //this.toDotPDFFile("tree.pdf");
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (this.toSexp().equals("()")) {
            LeafNode initNode = new LeafNode(metadata, bufferManager, new ArrayList<>(),
                    new ArrayList<>(), Optional.empty(), lockContext);

            updateRoot(initNode);
        } else {
            String message = String.format("You cannot bulkload an non-empty tree.");
            throw new BPlusTreeException(message);
        }

        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> pushKey = this.root.bulkLoad(data, fillFactor);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator (LeafNode start, DataBox key) {
            this.currNode = start;
            this.key = key;
            Iterator<RecordId> currRids = currNode.scanGreaterEqual(key);
        ...
        public boolean hasNext() {
            ...
            if (currRids.hasNext()) {
                ...
                LeafNode temp = currNode;
                while (temp.getRightSibling().isPresent()) {
                    if (temp.getRightSibling().get().getRids().size() > 0) {
                        return true;
                    }
        ...
        public RecordId next() {
            ...
            if (this.hasNext()) {
                if (currRids.hasNext()) {
                    ...
                    while (this.hasNext()) {
                        LeafNode temp = currNode.getRightSibling().get();
                        this.currNode = temp;
                        this.currRids = currNode.scanAll();
                        if (this.currRids.hasNext()) {
                            return currRids.next();
                        }
                    }
                    return null;
