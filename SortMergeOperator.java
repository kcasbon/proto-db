    ...
    private class SortMergeIterator implements Iterator<Record> {
        ...
        */
        ...
        private SortMergeIterator() {
            ...
            prepareLeft(TransactionContext.getTransaction(), getLeftSource(), getLeftColumnName());
            prepareRight(TransactionContext.getTransaction(), getRightSource(), getRightColumnName());
        ...
        private Record fetchNextRecord() {
            ...
            if (!marked) {
                while (compare(leftRecord, rightRecord) < 0) { // left < right
                    // advance left
                    leftRecord = leftIterator.next();
                }
                while (compare(leftRecord, rightRecord) > 0) { // right < left
                    // advance right
                    rightRecord = rightIterator.next();
                }
            ...
            if (compare(leftRecord, rightRecord) == 0) {
                Record mergeRec = leftRecord.concat(rightRecord);
